...
class KnxMap:
            ...
            while True:
                ...
                if response and isinstance(response, KnxDescriptionResponse):
                    target_report = KnxTargetReport(
                        ...
                        project_install_identifier=response.dib_dev_info.get(
                            "project_install_identifier"
                        ),
                        supported_services=[
                            KNX_SERVICES[k]
                            for k, v in response.dib_supp_sv_families.get(
                                "families"
                            ).items()
                        ],
                        bus_devices=[],
                    ...
                    if self.configuration_reads:
                        ...
                        (
                            ...
                            functools.partial(
                                KnxTunnelConnection,
                                future,
                                connection_type=_CONNECTION_TYPES.get(
                        ...
                        if connected:
                            ...
                            if count and count.data:
                                ...
                                conf_response = (
                                    yield from bus_protocol.configuration_request(
                                        target,
                                        object_type=11,
                                        num_elements=count,
                                        property=OBJECTS.get(11).get(
                            ...
                            if count and count.data:
                                ...
                                conf_response = (
                                    yield from bus_protocol.configuration_request(
                                        ...
                                        object_type=0,
                                        num_elements=count,
                                        property=OBJECTS.get(0).get(
                                            "PID_MANUFACTURER_ID"
                                        ),
                                    )
                                )
                                if conf_response and conf_response.data:
                                    target_report.manufacturer = (
                                        knxmap.utils.get_manufacturer_by_id(
    ...
    def _knx_search_worker(self):
        ...
        try:
            ...
            protocol = KnxGatewaySearch(
                multicast_addr=self.multicast_addr, port=self.port
            )
    ...
    def _search_gateways(self):
        ...
        yield from asyncio.ensure_future(
            asyncio.Task(self._knx_search_worker(), loop=self.loop)
        )
        self.t1 = time.time()
    ...
    @asyncio.coroutine
    def search(
        self, search_timeout=5, iface=None, multicast_addr="224.0.23.12", port=3671
    ):
        self.iface = iface
    ...
    def brute(self, targets=None, bus_target=None, full_key_space=False):
        ...
        tasks = [
            asyncio.Task(
                self.bruteforce_auth_key(t, bus_target, full_key_space), loop=self.loop
            )
            ...
            while True:
                ...
                if not alive:
                    ...
                    (
                        ...
                        desc_type,
                        desc_version,
                    ) = knxmap.utils.parse_device_descriptor(dev_desc)
                    device_state = None
                    ...
                    if desc_type > 1:
                        ...
                        if isinstance(manufacturer, (str, bytes, bytearray)):
                            ...
                            manufacturer = knxmap.utils.get_manufacturer_by_id(
                                manufacturer
                            )

                        # Read the device state
                        device_state_data = yield from protocol.apci_memory_read(
                            target, memory_address=0x0060
                        )
                        ...
                        serial = yield from protocol.apci_property_value_read(
                            target, property_id=DEVICE_OBJECTS.get("PID_SERIAL_NUMBER")
                        )
                        ...
                        for object_index, props in OBJECTS.items():
                            ...
                            for k, v in props.items():
                                ret = yield from protocol.apci_property_value_read(
                                    target, property_id=v, object_index=object_index
                                )
                                if ret:
                        ...
                        manufacturer = yield from protocol.apci_memory_read(
                            target, memory_address=0x0104, read_count=1
                        )
                        if isinstance(manufacturer, (str, bytes, bytearray)):
                            manufacturer = int.from_bytes(manufacturer, "big")
                            manufacturer = knxmap.utils.get_manufacturer_by_id(
                        ...
                        ret = yield from protocol.apci_memory_read(
                            target, memory_address=0x0105, read_count=2
                        )
                        if ret:
                            properties["Device Type"] = codecs.encode(ret, "hex")

                        ret = yield from protocol.apci_memory_read(
                            target, memory_address=0x0101, read_count=3
                        )
                        if ret:
                            properties["ManData"] = codecs.encode(ret, "hex")

                        ret = yield from protocol.apci_memory_read(
                            target, memory_address=0x0108, read_count=1
                        )
                        if ret:
                            properties["CheckLim"] = codecs.encode(ret, "hex")

                        ret = yield from protocol.apci_memory_read(
                            target, memory_address=0x01FE, read_count=1
                        )
                        if ret:
                        ...
                        for _ in range(51):
                            ret = yield from protocol.apci_memory_read(
                                target, memory_address=start_addr, read_count=5
                            )
                            if ret:
                    ...
                    if desc_type > 1 and not self.ignore_auth:
                        auth_level = yield from protocol.apci_authenticate(
                            target, key=self.auth_key
                        )
                        if auth_level > 0:
                            yield from protocol.tpci_disconnect(target)
                            queue.task_done()
                            LOGGER.error(
                                f"Invalid authentication key for target {target}"
                            )
                            continue

                    ret = yield from protocol.apci_memory_read(
                        target, memory_address=group_address_table, read_count=1
                    )
                        ...
                        if address_table:
                            ...
                            for addr in [ga[i : i + 2] for i in range(0, len(ga), 2)]:
                                properties["Group Addresses"].append(
                                    knxmap.utils.parse_knx_group_address(
                                        int.from_bytes(addr, "big")
                                    )
                                )
    ...
    def _tunnel_connection(self, knx_gateway):
        ...
        if connected:
            ...
            self.bus_connections[knx_gateway.host].append(
                {"transport": transport, "protocol": bus_protocol}
            )
    ...
    def _bus_scan(self, knx_gateway, bus_targets):
        ...
        connectors = [
            asyncio.Task(self._tunnel_connection(knx_gateway))
            for _ in range(connections)
        ]
        yield from asyncio.wait(connectors)
        LOGGER.info(
            "Established %d connections to target %s"
            % (len(self.bus_connections[knx_gateway.host]), knx_gateway.host)
        )
        workers = [
            asyncio.Task(
                self._knx_bus_worker(
                    c.get("transport"), c.get("protocol"), knx_gateway
                ),
                loop=self.loop,
            )
    ...
    def scan(
        ...
        if self.medium == "net":
            ...
            if bus_targets and self.knx_gateways:
                ...
                bus_scanners = [
                    asyncio.Task(
                        self._bus_scan(knx_gateway=g, bus_targets=bus_targets),
                        loop=self.loop,
                    )
                    for g in self.knx_gateways
                ]
            ...
            if USB_SUPPORT:
                ...
                from knxmap.usb.core import KnxHidReport, KnxUsbTransport

                try:
                    transport = KnxUsbTransport(vendor_id=0x147B, product_id=0x5120)
                except OSError:
                    LOGGER.error(
                        "Could not open USB device (try running KNXmap with superuser privileges)"
                    )
                    return

                frame = KnxEmi1Frame(knx_source="0.0.0", knx_destination="1.1.2")
                report = KnxHidReport(
                    message_code=0x11, protocol_id=0x01, frame=frame.pack()
                )
    ...
    def group_writer(
        ...
        if routing:
            ...
            if "KNXnet/IP Routing" not in knx_gateway.supported_services:
                LOGGER.error(
                    "KNX gateway {gateway} does not support Routing".format(
    ...
    def apci(self, target, desc_timeout=2, desc_retries=2, iface=False, args=None):
        ...
        workers = [
            ...
            for _ in range(
                self.max_workers
                if len(self.targets) > self.max_workers
        ...
        if connected:
            if args.apci_type == "Memory_Read":
                ...
                if alive:
                    ...
                    if not isinstance(memory_address, int) or not isinstance(
                        memory_data, bytes
                    ):
                        try:
                            memory_address = int(memory_address, 16)
                ...
                if alive:
                    ...
                    data = yield from protocol.apci_key_write(
                        target, level=args.auth_level, key=new_auth_key
                    )
                    protocol.tpci_disconnect(target)
                    if not data:
                        LOGGER.debug("No data received")
                    else:
                        LOGGER.info(f"Authorization level: {data}")
                ...
                alive = yield from protocol.tpci_connect(target)
                if alive:
                    data = yield from protocol.apci_authenticate(target, key=auth_key)
                    protocol.tpci_disconnect(target)
                ...
                if alive:
                    ...
                    if isinstance(data, (type(None), type(False))):
                        ...
                        LOGGER.info(f"Individual address: {data}")
                ...
                if alive:
                    ...
                    if dev_type > 1:
                        ...
                        if auth_level > 0:
                            ...
                            return
                    data = yield from protocol.apci_memory_read(
                        target, memory_address=0x0060, read_count=args.read_count
                    )
                    if not data:
                        LOGGER.debug("No data received")
                    else:
                        ...
                        if args.toggle:
                            if run_state.get("PROG_MODE"):
                                run_state = CemiFrame.pack_cemi_runstate(
                                    ...
                                    transport_layer_active=run_state.get(
                                        "TRANSPORT_LAYER"
                                    ),
                                    app_layer_active=run_state.get("APP_LAYER"),
                                    serial_interface_active=run_state.get(
                                        "SERIAL_INTERFACE"
                                    ),
