...
class DataRequest:
    ...
    def __repr__(self):
        return (
            "%s knx_source: %s, knx_destination: %s, tpci_type: %s, apci_type: %s, "
            "destination_type: %s"
            % (
    ...
    def pack(self):
        ...
        data_request.extend(
            struct.pack(
                "!B",
                self.pack_npci(
                    data_len=data_len, destination_type=self.destination_type
                ),
            )
        )
        if self.tpci_type:
            tpci = Tpci(tpci_type=self.tpci_type, tpci_sequence=self.tpci_sequence)
            tpci = tpci.pack()
            if self.tpci_type == "UCD":
                tpci |= (
                    TPCI_UNNUMBERED_CONTROL_DATA_TYPES.get(self.tpci_control_type) << 0
                )
            elif self.tpci_type == "NCD":
                tpci |= (
                    TPCI_NUMBERED_CONTROL_DATA_TYPES.get(self.tpci_control_type) << 0
                )
        if self.apci_type is not None:
            apci = Apci(apci_type=self.apci_type, apci_data=self.apci_data)
            apci = apci.pack()
            apci |= ((tpci >> 2) & 1) << 10
            apci |= ((tpci >> 3) & 1) << 11
            apci |= ((tpci >> 4) & 1) << 12
...
class ExtendedDataRequest:
    ...
    def __init__(
        ...
        if message:
            ...
            if tpci_type:
                assert (
                    tpci_type in CEMI_TPCI_TYPES.keys()
                ), f"Invalid TPCI type {tpci_type}"
            ...
            if apci_type:
                assert (
                    apci_type in CEMI_APCI_TYPES.keys()
                ), f"Invalid APCI type {apci_type}"
    ...
    @staticmethod
    def pack_control_field(
        confirm=False,
        acknowledge_req=False,
        priority=0x00,
    ...
    def pack(self):
        ...
        data_request.extend(
            struct.pack(
                "!B",
                self.pack_extended_control_field(
                    hop_count=self.routing_count, address_type=self.destination_type
                ),
            )
        )
        data_request.extend(struct.pack("!H", self.knx_source))
