...
class KnxMessage:
    ...
    def _parse_knx_body_hpai(self, message):
        try:
            ...
            (
                ...
                self.body["hpai"]["port"],
            ) = struct.unpack("!BBHH", message[:8])
            self.body["hpai"]["ip_address"] = socket.inet_aton(
                self.body["hpai"]["ip_address"]
            )
            return message[8:]
        except struct.error as e:
    ...
    def _unpack_dib_dev_info(self, message):
        ...
        dib_dev_info["device_status"] = CemiFrame.unpack_cemi_runstate(
            self._unpack_stream("!B", message)
        )
        dib_dev_info["knx_address"] = self.parse_knx_address(
        ...
        dib_dev_info["knx_mac_address"] = self.parse_mac_address(
            self._unpack_stream("!6s", message)
        )
