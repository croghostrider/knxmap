...
class KnxEmi1Frame:
    def __init__(
        self, data=None, message_code=0x11, knx_source=None, knx_destination=None
    ):
        self.message_code = message_code
        self.knx_source = (
            knxmap.utils.pack_knx_address(knx_source)
            if isinstance(knx_source, (str, bytes))
            else knx_source
        )
        self.knx_destination = (
            knxmap.utils.pack_knx_address(knx_destination)
            if isinstance(knx_destination, (str, bytes))
            else knx_destination
        )
    ...
    def __repr__(self):
        return (
            "EMI1 Frame message_code: {message_code}, source: {source}, "
