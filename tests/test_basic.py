...
from knxmap.targets import KnxTargets, Targets
...
class BasicTests(unittest.TestCase):
    ...
    def test_scan_bus_device(self):
        ...
        try:
            self.loop.run_until_complete(
                self.knxmap.scan(bus_targets=bus_targets.targets)
            )
        except KnxTunnelException as e:
            self.fail(e.message)
    ...
    def test_brute_default(self):
        ...
        try:
            self.loop.run_until_complete(
                self.knxmap.brute(bus_target=bus_targets.targets, full_key_space=False)
            )
        except KnxTunnelException as e:
            self.fail(e.message)

    def test_apci_memory_read(self):
        args = Args(apci_type="Memory_Read", memory_address=0x0116)
        try:
            self.loop.run_until_complete(self.knxmap.apci(target="1.1.2", args=args))
        except KnxTunnelException as e:
            self.fail(e.message)
